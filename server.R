...
folding_cal <- function(all_seqs = c()){
		fasta <- list()
		id <- c()
		for(j in 1:length(all_seqs)){
			a <- s2c(all_seqs[j])
			fasta <- c(fasta,list(a))
			id <- c(id,paste("tmp",j,sep=""))
		}
	
		wd <- getwd();
		setwd("C:/Users/luze/ViennaRNA Package/");
		write.fasta(fasta,id,file.out="tmpseq.fasta");
		shell(paste("RNAfold.exe", "<tmpseq.fasta>tmpseq.rnafold.txt"));
		...
		shell("del tmpseq.rnafold.txt");
		shell("del tmpseq.fasta");
		shell("del *.ps");
		setwd(wd);
...
shinyServer(function(input, output) {
	...
	v <- reactiveValues(data1 = NULL,data2 = NULL,data3 = NULL)
	...
	output$design <- renderPrint({
		...
		for(i in 1:length(inSeq)){
			...
			if(method1 == 'None'){ target_tail <- substr(inSeq[i],49,nchar(inSeq[i])) }
		...
		for(i in 1:num_seq){
			print(paste(">",target_id[i],sep=""))
			print(target_array[i],sep="")
		}
	...
	output$noptimize <- renderTable({
		...
		for(i in 1:length(inSeq)){
			...
			colnames(deltaG_tmp) <- c("SeqID","Seq","MFE of 6","GC of 6","MFE of 16","GC of 16","MFE of total length","GC of total length")
